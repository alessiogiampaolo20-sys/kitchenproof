...
| 2026-07-19 | §5 Supabase region | Production project runs in `eu-west-1` (Ireland) instead of `eu-central-1` (Frankfurt) | User choice at project creation, kept deliberately: both are EU regions, so the §17 GDPR/EU-processing requirement holds unchanged |
...
| 2026-07-21 | §14 provider transport | ClaudeProvider uses `messages.stream()` + `finalMessage()` + own Zod parse (was non-streaming `messages.parse`) with backoff on 429/5xx; AI-triggering pages export `maxDuration = 300` | Prod bug: SDK rejects non-streaming calls with large `max_tokens` ("Streaming is required… >10 min"); fixture provider in CI never exercised the real path. Verified live: official skema PDF → 26 rows, 53 s |
| 2026-07-21 | Vercel deploy | `vercel.json` pins `regions: ["dub1"]` | Functions ran in default `iad1` (US) with the DB in eu-west-1 — every query crossed the Atlantic (verified via `x-vercel-id: fra1::iad1`); dub1 sits next to the DB |
| 2026-07-21 | Auth signup | Email-confirmation flow made explicit: `emailRedirectTo` from request origin → `/auth/confirm` route (handles `code` + `token_hash`), "check your email" state, `email_not_confirmed` login error. Requires Supabase Site URL = production URL | Confirmation mails pointed at the default Site URL (localhost) and the app redirected to /login unexplained; user keeps confirm-email ON deliberately as an approval gate |
| 2026-07-21 | §15 operator shell | SiteNav is a client component mounted once in the `[site]` layout (usePathname active state) + sticky header with home link, profile menu (identity/role/locale/sign-out) and quick-add FAB | Per-page server nav re-rendered at scroll 0 on each tap → bar jumped, second taps mis-hit; identity/role and a home path were missing entirely (user feedback round 1) |
| 2026-07-21 | `[site]` layout stays data-free | Profile identity (user + profile + membership + org) loads via `loadSiteIdentity` server action when the menu is opened, not in the layout | Fetching it in the layout added an auth round-trip + 3 queries to **every** page render: measured 1:05 → 3:01 on one e2e spec, with timeouts. Layout now only reads params + the guest-lock cookie |
| 2026-07-28 | §1.4 chain | `orders` + `productions` (thin event owning the batches it created) + `production_batches`/`production_orders`; ingredients linked as **batches**, so one edge reaches receipt → invoice → supplier; logging a production attaches that day's heat/cooling records; investigation view renders both directions with a verdict line | Traceability could not answer either question that matters: what fed this order and were the standards met, or which clients must be contacted. The spreadsheet joins orders to purchases by shared date — the production day is the missing relation. Verdict never calls an unmonitored chain clean, and clients without contact details stay in the list (they need the phone call) |
| 2026-07-28 | §3.3 product vs ambient | Pack limits carry `measurementKind` + its own `sourceRef`; `task_completions.measurement_kind`; a reading whose kind differs from the limit's is **not scored** — interactively the operator is asked and the check screen states which temperature to take, on outbox replay the record is kept with kind NULL rather than rejected. Pack 2026.07 → 2026.08 | DK-HYGIEJNE kap. 26.2 (p. 57) verified in `docs/reference/`: the hygiene order's limits are ambient, the animal-origin regulation and frozen-food order are product temperatures. Judging air against a food limit passes what should fail and vice versa. Gotcha found by the new e2e: editing a control point rebuilt `limit_json` from form fields and silently dropped the kind |
| 2026-07-27 | §3.5 operating calendar | `sites.operating_pattern` (null = open every day) + `site_operating_days` (attributed, audited, no delete, server-set `confirmed_at`); pure `operating-days.ts`; materializer skips closed days, daily checks drop, weekly/monthly **roll** to the next open day keeping their wall-clock time; cron silent on closed days; inspector heat-map renders closed days as closed | A catering site does not cook daily: every closed day filled with false overdue, and a gap in the records reads to an inspector as a missing record while "closed, no production" is a complete answer. Rule is deliberately asymmetric — only an explicit confirmation or the pattern closes a day; uncertainty asks rather than suppressing a real obligation |
| 2026-07-27 | §10.1 inspector link | `resolve_inspector_link` returns `link_id` (audit names the link actually used, not the newest for the site); `revoked_at`/`revoked_by` + revoke action; manager picks 4/8/24 h; view audit records IP + user-agent; `force-dynamic` explicit; e2e proves a record made **after** issuing appears in a fresh anonymous context | The link was live but unproven (the old spec created its record first), the owner could neither see nor stop a visit, and with two live links the audit blamed the wrong one (docs/audit.md §3.4). Gotcha: a `"use server"` module may only export async functions — the hours constant lives in `link-options.ts` |
| 2026-07-27 | §17 fail-loud policy | Service-role client throws on a missing/blank key; service-client reads go through `unwrap()`; `CronReport.errors` + HTTP 500 when any step fails; `server-only` aliased to a stub in vitest so server modules are testable | Prod ran 5 days with a broken `SUPABASE_SERVICE_ROLE_KEY`: cron answered `{"sitesMaterialized":0}` with HTTP 200 and the inspector saw empty tabs, because `!` assertions and dropped `error` values turn a config fault into a silent no-op (docs/audit.md §3). Third occurrence of this class |
| 2026-07-21 | §20 onboarding | `/app/[site]/setup` getting-started checklist (pure `computeOnboarding`): 3 basics (approved programme, equipment, manager PIN) + 3 takeover steps (catalog, opening stock, existing documents) | Existing businesses don't start from zero (user feedback round 1); every takeover step reuses an existing flow (§7.5 import, quick receive, §10.2 documents) so nothing new can back-date records |
